...
function LocalActivity(args = {}) {
    ...
    function toFITjs(args = {}) {
        ...
        const activity_start_time = first(laps)?.start_time ?? first(records).timestamp;
...
function Session(args = {}) {
    ...
    const start_time = args.start_time ?? first(laps).timestamp;
    const timestamp = args.timestamp ?? last(laps)?.timestamp;
    const total_elapsed_time = args.total_elapsed_time ??
          type.timestamp.elapsed(start_time, timestamp);
