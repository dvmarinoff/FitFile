...
const unsigned = ['uint8', 'uint16', 'uint16z', 'uint32', 'uint32z', 'uint64', 'uint64z'];
...
function setView(
    ...
    if(useBaseType) {
        if(value < 0 && unsigned.includes(type)) {
            console.warn(`fit: encode: passing-negative-to-unsigned: ${value} to: ${type} at: ${i}`);
            value = 0;
        }
